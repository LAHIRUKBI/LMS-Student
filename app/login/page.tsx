...
export default function LoginPage() {
  ...
  // State for the Room Light (false = Dark Mode/Torch On, true = Light Mode/Torch Off)
  ...
  // Dynamic Styles — Dark Mode colors MATCHED with dashboard (from-slate-950 to-slate-900)
  const theme = {
    bg: isRoomLightOn
      ? "bg-slate-100"
  ...
  return (
    <div className={`relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden transition-colors duration-1000 px-4 ${theme.bg}`}>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 60% 55% at 32% 78%, rgba(255, 255, 255, 0.14) 0%, rgba(255, 255, 255, 0.04) 40%, transparent 75%)",
        }}
      />

      {/* ===== Subtle Vignette (Dark Mode) ===== */}
      <div
        className={`absolute inset-0 pointer-events-none transition-opacity duration-1000 z-0 ${
          isRoomLightOn ? "opacity-0" : "opacity-100"
        }`}
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center mt-4">
        <div
        ...
        >
          ...
          <p className={`mt-6 text-center text-sm transition-colors duration-1000 ${theme.textSecondary}`}>
            ...
            <Link href="/register" className={`font-semibold underline-offset-4 transition-colors hover:underline ${isRoomLightOn ? "text-indigo-600 hover:text-indigo-700" : "text-white hover:text-slate-200"}`}>
              Register
            </Link>
          </p>
        </div>
